...
class Account(ContribChargenAccount):
    ...
    * Hook methods
    ...
    # Localizzazione italiana dello splash OOC (Fase C, quarta tornata).
    # Sovrascriviamo sia il template sia at_look() perche' l'header, il
    # blocco sessioni e l'elenco personaggi sono costruiti come stringhe
    # dirette dentro at_look() nell'originale, non solo nel template.
...
{sessions}
  ...
  |whelp|n - altri comandi
  |wpublic <testo>|n - parla sul canale pubblico
  |wcharcreate <nome> [=descrizione]|n - crea un nuovo personaggio
  |wchardelete <nome>|n - elimina un personaggio
  |wic <nome>|n - entra in gioco come quel personaggio (|wooc|n per tornare qui)
  |wic|n - entra in gioco con l'ultimo personaggio controllato.
...
""".strip()
    ...
    def at_look(self, target=None, session=None, **kwargs):
        """Versione italiana dello splash out-of-character."""

        if target and not is_iter(target):
            if hasattr(target, "return_appearance"):
                return target.return_appearance(self)
            else:
                return f"{target} non ha un aspetto di gioco."
        ...
        txt_header = f"Account |g{self.name}|n (sei fuori dal personaggio, OOC)"
        ...
        txt_sessions = "|wSessioni connesse:|n\n" + "\n".join(sess_strings)

        if not characters:
            txt_characters = "Non hai ancora un personaggio. Usa |wcharcreate|n."
        else:
            max_chars = (
                "illimitato"
                    ...
                    for sess in csessions:
                        ...
                        if sess and sid:
                            char_strings.append(
                                ...
                                f"(giocato da te nella sessione {sid})"
                            )
                        else:
                            char_strings.append(
                                f" - |R{char.name}|n [{', '.join(char.permissions.all())}] "
                                "(giocato da qualcun altro)"
                            )
                else:
                    char_strings.append(f" - {char.name} [{', '.join(char.permissions.all())}]")

            txt_characters = (
                f"Personaggio/i disponibile/i ({ncars}/{max_chars}, |wic <nome>|n per giocare):|n\n"
                + "\n".join(char_strings)
            )
