...
class ObjectParent:
    ...
    def get_numbered_name(self, count, looker, **kwargs):
        """
        Evennia usa di serie la libreria inglese `inflect` per anteporre
        "a"/"an" e per pluralizzare (es. "an apple", "three apples"): non e'
        traducibile via gettext perche' non passa da stringhe di sistema, e
        l'inglese non ha comunque il genere che servirebbe in italiano
        ("un pugnale" / "una spada"). Piu' semplice ed efficace: niente
        articolo automatico, il nome nudo (comune anche in molti MUD
        italiani per liste di oggetti); il plurale resta un placeholder
        "N nome" finche' non serve davvero (nessun oggetto impilabile
        ancora in gioco).
        """
        key = kwargs.get("key", self.get_display_name(looker))
        singolare = key
        plurale = key if count <= 1 else f"{count} {key}"
        if kwargs.get("return_string"):
            return singolare if count in (0, 1) else plurale
...
class OggettoLuce(Object):
    """
    Oggetto-luce (torcia/lanterna/candela - Fase K, sedicesima tornata,
    vedi world/illuminazione.py). helps/light.txt: "While holding a
    light object automatically ignites it, and removing it
    automatically extinguishes it" - i due hook sotto riproducono
    esattamente questo comportamento di default; i comandi LIGHT/
    EXTINGUISH (commands/cthulhu_illuminazione.py) permettono di
    scavalcarlo manualmente (es. lasciarlo acceso a terra come fonte
    fissa, o spegnerlo mentre lo si tiene in mano).

    Semplificazione dichiarata: "holding" e' qui semplicemente
    "presente nell'inventario del personaggio", senza richiedere che
    sia impugnato/indossato in uno slot (coerente con come gia'
    funzionano cibo/bevande in questo porting, world/sopravvivenza.py)
    - la fonte non descrive un vincolo piu' stretto. Passare l'oggetto
    con GIVE non lo spegne (non passa da at_get/at_drop): resta acceso
    o spento cosi' com'era, dato che non e' mai stato davvero "rimosso"
    da una mano nel mezzo del passaggio.
    """

    def at_object_creation(self):
        super().at_object_creation()
        self.db.luce = True
        self.db.luce_accesa = True  # "holding it automatically ignites it" - appena comprato/creato e' gia' in mano

    def at_get(self, getter, **kwargs):
        super().at_get(getter, **kwargs)
        self.db.luce_accesa = True

    def at_drop(self, dropper, **kwargs):
        super().at_drop(dropper, **kwargs)
        self.db.luce_accesa = False


class Corpse(ObjectParent, DefaultObject):
    """
    Cadavere lasciato da un personaggio o da un NPC morto (M3). Decade da
    solo dopo 15 minuti reali (world.combat.DURATA_DECADIMENTO_MINUTI,
    confermato dalla fonte). Alla morte di un giocatore (world/combat.py,
    _morte_personaggio) il cadavere viene creato nella stanza MORGUE della
    professione della vittima (non dove e' morta, salvo che non abbia una
    morgue assegnata) e contiene tutto cio' che portava addosso: se non lo
    recuperi (GET <oggetto> CADAVERE) prima che decada, il contenuto cade
    comunque nella stanza (non sparisce) - da li' in poi e' libero per
    chiunque lo trovi, come confermato dal sito originale.
    """

    def at_object_creation(self):
        super().at_object_creation()
        # world/combat.py:crea_cadavere() lo sovrascrive sempre subito dopo
        # con DURATA_DECADIMENTO_MINUTI (15, il valore confermato dalla
        # fonte) - questo e' solo un default di sicurezza per un Corpse
        # creato senza passare da li' (nessun caso reale in questo
        # porting, vedi il docstring della classe).
        self.db.decadimento_minuti = 15
        self.locks.add("get:false()")  # non raccoglibile come oggetto qualsiasi
        self.scripts.add("typeclasses.scripts.CorpseDecayScript", key="decadimento")
